...
在 C++ 多线程里，==**死锁通常是多个线程互相等待对方持有的锁导致的**==。避免死锁最常用的方法是统一加锁顺序，保证多个线程按相同顺序获取锁；同时尽量减少嵌套锁和锁持有时间，避免在临界区里做耗时操作。对于多把锁的场景，优先使用 `std::lock` 或 `std::scoped_lock` 来一次性加锁，避免手动加锁顺序不一致。另外要使用 `lock_guard`、`unique_lock` 这类 RAII 方式管理锁，防止异常或提前 return 时忘记释放。
